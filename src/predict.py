...
def calc_score(test_num, trained_model, use_model_epoch, config_path, plot_results=False, input_as_random_noize=False,  make_notelvl_from_framelvl=False, verbose=True):
    ...
    model = TabEstimator(mode, use_custom_decimation_func, use_conv_stack, n_bins, hop_length, down_sampling_rate, encoder_heads=encoder_heads,
    ...
    note_sum_onset_precision, note_sum_onset_recall, note_sum_onset_f1 = 0, 0, 0   # NEW
    ...
    for npz_filename in tqdm.tqdm(test_data_list):
        ...
        note_onset_gt = torch.from_numpy(npz_file["tab_onset"])
        ...
        # prediction
        with torch.no_grad():
            frame_pred, note_pred, note_onset_pred, olens = model(     # NEW: note_onset_pred
        ...
        note_onset_pred = torch.squeeze(note_onset_pred, 0)
        argmax_index = np.argmax(note_onset_pred.detach().numpy(), axis=2)
        note_onset_pred = np.zeros((len(note_onset_pred), 6, 21))
        for note in range(len(note_onset_pred)):
            for string in range(6):
                note_onset_pred[note, string, argmax_index[note, string]] = 1

        frame_gt = frame_gt.detach().numpy()
        note_gt = note_gt.detach().numpy()
        note_onset_gt = note_onset_gt.detach().numpy()
        ...
        np.savez_compressed(npz_save_filename,
                            ...
                            note_tab_onset_gt=note_onset_gt,
        ...
        note_onset_pred = note_onset_pred[:, :, :-1].flatten()
        note_onset_gt = note_onset_gt[:, :, :-1].flatten()
        ...
        note_precision, note_recall, note_f1 = calculate_metrics(
            note_pred, note_gt)
        note_onset_precision, note_onset_recall, note_onset_f1 = calculate_metrics(   # NEW
        note_onset_pred, note_onset_gt)
        ...
        note_sum_onset_precision += note_onset_precision   # NEW
        note_sum_onset_recall += note_onset_recall         # NEW
        note_sum_onset_f1 += note_onset_f1                 # NEW
    ...
    note_avg_onset_precision = note_sum_onset_precision / len(test_data_list)   # NEW
    note_avg_onset_recall = note_sum_onset_recall / len(test_data_list)         # NEW
    note_avg_onset_f1 = note_sum_onset_f1 / len(test_data_list)   
    ...
    result = pd.DataFrame([[frame_avg_precision, frame_avg_recall, frame_avg_f1,
                        ...
                        note_avg_precision, note_avg_recall, note_avg_f1,
                        note_avg_onset_precision, note_avg_onset_recall, note_avg_onset_f1,
                      ...
                      columns=["frame_segment_avg_tab_p", "frame_segment_avg_tab_r", "frame_segment_avg_tab_f",
                               ...
                               "note_avg_onset_p", "note_avg_onset_r", "note_avg_onset_f",
...
def main():
    ...
    for test_num in range(6):
        ...
        score = calc_score(test_num, trained_model, use_model_epoch, config_path, plot_results=plot_results,
    ...
    result = pd.concat([result, result.describe()[1:3]])
