...
def main(midi_filename_list, kwargs):
    ...
    for midi_filename in midi_filename_list:
        ...
        # process note-wise F0 (pitch-space ground truth, used by the
        # tab-mode F0-from-tab evaluation metrics / TDR in predict.py --
        ...
        split_save(npz_path, cqt, tab, tab_onset, frame_tab, frame_tab_onset, F0, frame_F0,
                   tempo, note_resolution)
        print("finished", os.path.split(npz_path)[1])


def split_save(npz_path, cqt, tab, tab_onset, frame_tab, frame_tab_onset, F0, frame_F0, tempo, note_resolution):
    ...
    for n_4bars in range(note_len_4bars):
        ...
        assert split_cqt.shape[0] == split_frame_tab.shape[0] == split_frame_tab_onset.shape[0] == split_frame_F0.shape[0]
