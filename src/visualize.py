...
def visualize(npz_filename_list, kwargs):
    ...
    for npz_filename in npz_filename_list:
        ...
        if mode == "F0":
            ...
            note_tab_onset_pred = npz_file["note_tab_onset_pred"]   # CHANGED
        ...
        n_subplots = (
            3 + encoder_layers * encoder_heads) if mode == "F0" else (6 + encoder_layers * encoder_heads)   # CHANGED: 5 -> 6
        ...
        if mode == "F0":
            ...
            plt.xticks([i for i in range(0, n_note + 1, note_resolution)],
            ...
            # try to load note-wise tab onset from npz if available
            note_tab_onset = None
            if "tab_onset" in npz_file:        #these are redundatnt steps
                note_tab_onset = npz_file["tab_onset"]
            elif "note_tab_onset" in npz_file:
                note_tab_onset = npz_file["tab_onset"]
            ...
            # if a predicted note-onset array exists in the npz use it, otherwise pass None
            pred_note_onset = None
            if "note_tab_onset" in npz_file:   # these are redundant steps we have already changed name
                pred_note_onset = npz_file["note_tab_onset"]
            elif "tab_onset_pred" in npz_file:
                pred_note_onset = npz_file["tab_onset_pred"]
