...
- [midi_to_numpy.py]
  convert midi files created from [jams_to_midi.py] to npz files.
  
- [tab_estimator_model.py]
...
- [train.py]
  train the model defined in [tab_estimator_model.py]
...
In all of the following instructions, we will assume that you are in `FinalSrcCode/` directory.

### Make input and output features
Run [src/jams_to_midi.py] 
This will convert the jams files provided by GuitarSet into midi files.
Next, run [src/midi_to_numpy.py]
...
### Training
Check [src/config.yaml] and run [src/train.py].
Running [src/train.py] will copy the [src/config.yaml] to `model/date_and_time/`.
The format of `date_and_time` is YYYYMMDDhhmm.

Trained models will be saved in `model/date and time/` every 32 epochs. 

### Predict and evaluate
Run [src/predict.py] with arguments for designating the model. 
The first argument is `date_and_time`, and the second argument is `epochs`.
ex) `python3 src/predict.py 202601012359 64`
This will load the trained model, predict, calculate metrics, and save the results to `result/tab`.

### Visualize the predictions
Run [src/visualize.py] with arguments for designating the model.
ex) `python3 src/visualize.py 202601012359 64`
This will visualize the results and save these to `result/*/visualize/`.

### Sample files
epoch64.model , resultPianoROll_folder , sample_metrics.csv are incomplete trained results only after 64 epoch. 
